...
describe("Network Link Functions", () => {
  ...
  describe("Basic Link Operations", () => {
    ...
    test("should set link type", () => {
      ...
      model.addNode("Node1", NodeType.Junction);
      model.addNode("Node2", NodeType.Junction);
      const linkIndex = model.addLink("P1", LinkType.Pipe, "Node1", "Node2");

      // Verify initial type
      expect(model.getLinkType(linkIndex)).toEqual(LinkType.Pipe);

      // Change to PRV
      model.setLinkType(linkIndex, LinkType.PRV, ActionCodeType.Unconditional);
      expect(model.getLinkType(linkIndex)).toEqual(LinkType.PRV);

      // Change back to pipe
      model.setLinkType(linkIndex, LinkType.Pipe, ActionCodeType.Conditional);
      expect(model.getLinkType(linkIndex)).toEqual(LinkType.Pipe);
    });
  });
